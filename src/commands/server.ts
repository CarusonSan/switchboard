...
const gameChoices = Object.values(games).map((g) => ({
  name: g.label,
  value: g.id,
}));
...
const data = new SlashCommandBuilder()
  ...
  .addSubcommand((sub) =>
    sub.setName("list").setDescription("List all game servers"),
  )
  .addSubcommand((sub) =>
    sub
      .setName("status")
  ...
  try {
    switch (subcommand) {
      ...
      case "stop": {
        ...
        await interaction.editReply(
          `Stopped \`${name}\`. World data is preserved.`,
        );
        break;
      }
      case "remove": {
        const name = interaction.options.getString("name", true);
        const deleteData =
          interaction.options.getBoolean("delete-data") ?? false;
        const removed = await provider.remove(name, deleteData);
        if (removed.dataDeleted) {
          await interaction.editReply(
            `Removed \`${name}\` and deleted its world data.`,
          );
      ...
      case "list": {
        const [servers, pool] = await Promise.all([
          provider.list(),
          provider.poolStatus(),
        ]);
        const lines =
        ...
        if (pool.length > 0) {
          ...
          const summary = [...byGame.entries()]
            .map(
              ([game, { ready, total }]) =>
                `${games[game]?.label ?? game} ${ready}/${total} ready`,
            )
            .join(", ");
